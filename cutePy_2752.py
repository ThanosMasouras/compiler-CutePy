#ATHANASIOS MASOURAS 2752 CSE42752
import sys
...
def open_table_file():
    global file_table
    try:
        file_table = open("symbol_table.txt", "r+")
    except FileNotFoundError:
        file_table = open("symbol_table.txt", "w+")
    file_table.truncate(0)

def save_lex_tokens_to_file():
	with open("lex_tokens.txt", "w") as f:
...
def save_symbol_table_to_file():
	with open("symbol_table.txt", "a") as f:
		...
		for scope in scopes:
			f.write(f"scope level: {scope.nested_level}\n")
			f.write(f" entities: ")
			for entity in scope.entities:
				if isinstance(entity, Function):
					f.write(f" {entity.name}/{entity.frameLength}, ")
				else:
